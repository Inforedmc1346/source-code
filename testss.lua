...
MainTab:AddSection({Name = "Boss"})
local BossSelect = MainTab:AddDropdown({
	Name = "Select Boss",
	Default = "",
	Options = GetBossTable(),
	Callback = function(vSelectBoss)
...
MainTab:AddButton({
	Name = "Refresh Player",
	Callback = function()
	    BossSelect:Refresh(GetBossTable(), true)
  	end    
})
MainTab:AddToggle({
	Name = "Kill Boss",
	Default = false,
	Callback = function(vKillBoss)
		KillBoss = vKillBoss
	end    
})
spawn(function()
    while wait() do
        if KillBoss and SelectBoss and (ReturnBosses(tostring(SelectBoss))) then
            if ReturnBosses(SelectBoss) then
                local v = ReturnBosses(SelectBoss)
                if Enemies:FindFirstChild(v.Name) then
                    repeat task.wait()
                        ...
                        v.HumanoidRootPart.CanCollide = false
                        NoClip = true
                    until not KillBoss or not SelectBoss or not v or not v:FindFirstChild(v.Name)
                    NoClip = false
                else
                    ToTween(v.HumanoidRootPart.CFrame * CFrame.new(0,30,0))
                    NoClip = true
